...
output_dir = "wild_fire_vtk_output"
os.makedirs(output_dir, exist_ok=True)

def write_vti(filename, data):
    GRID_SIZE = data.shape[0]
    data_3d = data.reshape((GRID_SIZE, GRID_SIZE, 1))
    imageToVTK(os.path.join(output_dir, filename), pointData={"Forest_State": data_3d})
...
def simulate_wildfire():
    ...
    for day in range(DAYS):
        ...
        write_vti(f"forest_day_{day:03d}", forest)
...
print(f"Serial simulation completed in {time.time() - start_time:.2f} seconds.")
